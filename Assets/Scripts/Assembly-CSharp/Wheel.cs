	...
	public void GetWheelDimensions(float radiusMargin, float widthMargin)
	{
		...
		if ((bool)mesh3)
		{
			...
			for (int i = 0; i < vertices.Length; i++)
			{
				...
				Vector2 vector2 = new Vector2(vector.x * child.localScale.x, vector.y * child.localScale.y);
				if (vector2.magnitude > num2)
				{
					Vector2 vector3 = new Vector2(vector.x * child.localScale.x, vector.y * child.localScale.y);
			...
			if ((bool)mesh2 && (bool)mesh)
			{
				...
				for (int j = 0; j < vertices2.Length; j++)
				{
					...
					Vector2 vector5 = new Vector2(vector4.x * child.localScale.x, vector4.y * child.localScale.y);
					if (vector5.magnitude > num2)
					{
						Vector2 vector6 = new Vector2(vector4.x * child.localScale.x, vector4.y * child.localScale.y);
						num2 = vector6.magnitude;
					}
					if (Mathf.Abs(vector4.z * child.localScale.z) > num)
					{
						num = Mathf.Abs(vector4.z * child.localScale.z);
					}
				}
