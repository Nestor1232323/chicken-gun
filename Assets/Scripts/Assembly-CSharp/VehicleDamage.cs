...
public class VehicleDamage : MonoBehaviour
{
	...
	private void OnCollisionEnter(Collision col)
	{
		...
		for (int i = 0; i < contacts.Length; i++)
		{
			...
			if (tr.InverseTransformPoint(colPoint.point).y > collisionIgnoreHeight && (int)GlobalControl.damageMaskStatic == ((int)GlobalControl.damageMaskStatic | (1 << colPoint.otherCollider.gameObject.layer)))
			{
				...
				DamageApplication(colPoint.point, col.relativeVelocity, maxCollisionMagnitude, colPoint.normal, colPoint, true);
			}
	...
	{
		DamageApplication(colPoint.point, colVel, float.PositiveInfinity, colPoint.normal, colPoint, true);
		FinalizeDamage();
	}
	...
	{
		DamageApplication(damagePoint, damageForce, float.PositiveInfinity, damageForce.normalized, nullContact, false);
		FinalizeDamage();
	}
	...
	private void DamageApplication(Vector3 damagePoint, Vector3 damageForce, float damageForceLimit, Vector3 surfaceNormal, ContactPoint colPoint, bool useContactPoint)
	{
		...
		for (int l = 0; l < deformColliders.Length; l++)
		{
			...
			for (int m = 0; m < colVertices[l].verts.Length; m++)
			{
				...
				{
					damagedCols[l] = true;
					colVertices[l].verts[m] += vector3 * num4 * Mathf.Min(num2 * 0.01f, num6) * num3;
				}
			}
		}
		for (int n = 0; n < displaceParts.Length; n++)
		{
			...
			transform3.position += vector3 * num4 * Mathf.Min(num2 * 0.01f, num6) * num3;
			...
			else if ((bool)transform3.parent.GetComponent<DetachablePart>())
			{
				...
				{
					component4.displacedAnchor += transform3.parent.InverseTransformDirection(vector3 * num4 * Mathf.Min(num2 * 0.01f, num6) * num3);
				}
			}
			...
			if ((bool)component5 && ((!component5.wheel.grounded && ignoreGroundedWheels) || !ignoreGroundedWheels))
			{
				...
				foreach (SuspensionPart movingPart in component5.movingParts)
				{
					...
					{
						movingPart.connectPoint += movingPart.connectObj.InverseTransformDirection(vector3 * num4 * Mathf.Min(num2 * 0.01f, num6) * num3);
					}
				}
			}
