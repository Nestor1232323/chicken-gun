	...
	public void Detach(bool makeJoint)
	{
		...
		if ((bool)parentBody)
		{
			...
			if (makeJoint && joints.Length != 0)
			{
				...
				{
					min = partJoint.minLimit,
					max = partJoint.maxLimit,
					bounciness = partJoint.bounciness
				};
				hinge.limits = limits;
				hinge.useSpring = partJoint.useSpring;
				JointSpring spring = new JointSpring
				{
					targetPosition = partJoint.springTargetPosition,
					spring = partJoint.springForce,
					damper = partJoint.springDamper
				};
				hinge.spring = spring;
